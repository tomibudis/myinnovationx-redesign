...
// Marketplace/Listings Preview Functionality
function initializeMarketplace() {
    const marketplaceItems = document.querySelectorAll('.marketplace-item');
    const browseMarketplaceBtn = document.querySelector('#marketplace-preview button');
    const viewAllLink = document.querySelector('#marketplace-preview a[href="#marketplace"]');
    
    // Add click handlers for marketplace items
    marketplaceItems.forEach((item, index) => {
        const viewDetailsBtn = item.querySelector('button');
        const itemTitle = item.querySelector('h3');
        
        // Make entire card clickable
        item.addEventListener('click', (e) => {
            // Don't trigger if clicking on the button itself
            if (e.target === viewDetailsBtn) return;
            
            handleMarketplaceItemClick(item, index);
        });
        
        // Add keyboard navigation
        item.setAttribute('tabindex', '0');
        item.setAttribute('role', 'button');
        item.setAttribute('aria-label', `View details for ${itemTitle ? itemTitle.textContent : 'marketplace item'}`);
    ...
    // Browse marketplace button handler
    ...
    if (viewAllLink) {
        viewAllLink.addEventListener('click', (e) => {
            e.preventDefault();
            handleViewAllMarketplace();
        });
    }
    
    // Add scroll animation for marketplace items
    initializeMarketplaceScrollAnimation();
}

function handleMarketplaceItemClick(item, index) {
    const itemTitle = item.querySelector('h3');
    const title = itemTitle ? itemTitle.textContent : 'Marketplace Item';
    ...
    setTimeout(() => {
        ...
        console.log(`Opening marketplace item ${index + 1}: ${title}`);
        
        // You could add modal functionality here or navigate to a details page
        showMarketplaceItemModal(item, index);
    }, 500);
}

function handleBrowseMarketplace() {
    showNotification('Opening full marketplace...', 'info');
    
    // Simulate loading
    setTimeout(() => {
        console.log('Navigating to full marketplace');
        // In a real application, this would navigate to the marketplace page
        showNotification('Marketplace loaded successfully!', 'success');
    }, 1000);
}

function handleViewAllMarketplace() {
    showNotification('Loading all marketplace listings...', 'info');
    
    // Simulate loading more items
    setTimeout(() => {
        console.log('Loading all marketplace listings');
        showNotification('All listings loaded!', 'success');
    }, 800);
}

function showMarketplaceItemModal(item, index) {
    // Create modal for marketplace item details
        ...
        <div class="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div class="p-6">
                <div class="flex justify-between items-start mb-4">
                    <h3 class="text-2xl font-bold text-gray-900">Marketplace Item Details</h3>
                ...
                <div class="space-y-4">
                    <p class="text-gray-600">This is a preview of the marketplace item details. In a real application, this would show comprehensive information about the solution, including:</p>
                    <ul class="list-disc list-inside space-y-2 text-gray-600">
                        <li>Detailed description and features</li>
                        <li>Company information and team</li>
                        <li>Pricing and partnership options</li>
                        <li>Contact information and next steps</li>
                        <li>Related solutions and recommendations</li>
                    </ul>
                    <div class="flex gap-3 pt-4">
                        <button class="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-colors" onclick="this.closest('.fixed').remove()">
                            Close
                        </button>
                        <button class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors" onclick="this.closest('.fixed').remove()">
                            Contact Company
                        </button>
                    </div>
                </div>
...
function initializeMarketplaceScrollAnimation() {
    const marketplaceSection = document.getElementById('marketplace-preview');
    if (!marketplaceSection) return;
    
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // Trigger animation for marketplace items
                const marketplaceItems = entry.target.querySelectorAll('.marketplace-item');
                marketplaceItems.forEach((item, index) => {
                    setTimeout(() => {
...
// Marketplace filtering functionality (for future enhancement)
function initializeMarketplaceFilters() {
    // This could be expanded to add filtering by sector, stage, etc.
    const filterButtons = document.querySelectorAll('.marketplace-filter-btn');
...
function filterMarketplaceItems(filter) {
    ...
    marketplaceItems.forEach(item => {
        ...
        if (filter === 'all' || itemSector === filter || itemStage === filter) {
            item.style.display = 'block';
            item.style.animation = 'marketplaceItemFadeIn 0.5s ease-out';
        } else {
